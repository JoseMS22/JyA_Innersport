...
def upgrade() -> None:
    ...
    # Usar SIEMPRE la conexión de Alembic, no el engine de la app
    bind = op.get_bind()
    inspector = inspect(bind)

    # Si la tabla no existe (caso muy raro), no hacemos nada
    try:
        existing_columns = [col["name"] for col in inspector.get_columns("direccion")]
    except NoSuchTableError:
        print("⚠ Tabla 'direccion' no existe, se omite migración de campos extra.")
        return
