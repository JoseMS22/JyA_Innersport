...
export function MainMenu() {
  ...
  // justo debajo de los otros useEffect, por ejemplo después del que detecta el scroll
  useEffect(() => {
    ...
    if (mobileMenuOpen) {
      ...
      // cleanup si el componente se desmonta con el menú abierto
      return () => {
        document.body.style.overflow = originalStyle;
      };
    } else {
      // cuando se cierra el menú, volvemos a la normalidad
      document.body.style.overflow = "";
    }
  }, [mobileMenuOpen]);


  // Cargar categorías para el menú (principales + secundarias)
  useEffect(() => {
    async function loadMenuCategorias() {
      try {
        ...
        setMenuCategorias(data); // 👈 sin filtros extra
      } catch (err) {
  ...
  function handleProfileClick() {
    // Siempre abre/cierra el dropdown, esté o no logueado
  ...
  const navItemClass = (active?: boolean) =>
    [
      ...
      "text-[#6b21a8]", // color morado principal
      active
        ? "bg-[#f3e8ff] border border-[#e9d5ff] shadow-sm" // pill activa
...
`;
  ...
  return (
    <>
          ...
          <div className="max-w-6xl mx-auto px-4 py-3">
            ...
            <div className="hidden md:flex items-center gap-4">
              ...
              <nav className="flex-1 flex items-center justify-center gap-4">
                ...
                {/* Dinámicos: categorías principales con subcategorías */}
        ...
        "
              ...
              <div className="relative flex items-center gap-3 flex-shrink-0">
                ...
                {!checking && profileMenuOpen && (
                  <div className="absolute right-10 top-full mt-2 w-44 rounded-xl border border-gray-200 bg-white shadow-lg py-2 text-xs">
                    {user ? (
                      <>
                        <button
                          onClick={() => {
                        ...
                        <button
                          onClick={() => {
                            setProfileMenuOpen(false);
                            router.push("/account/addresses"); // ajusta la ruta si se llama distinto
                          }}
                          className="w-full text-left px-3 py-2 hover:bg-gray-50 text-gray-700"
                        >
                          Direcciones
              ...
              <div className="flex-1">
                <SearchBar
                  ...
                  onSearch={(q) => {
                    ...
                    // Construimos los query params actuales
                    const params = new URLSearchParams(searchParams.toString());
                    params.set("buscar", query);
                    params.delete("pagina"); // reset paginación

                    // Siempre vamos al catálogo principal (/)
                    router.push(`/?${params.toString()}`);

                    // Cerramos la barra de búsqueda del menú
                    setSearchOpen(false);
                  }}
                />
              </div>
      ...
      {/* 🔴 MENÚ MÓVIL FULL SCREEN, FUERA DEL HEADER */}
      {mobileMenuOpen && (
